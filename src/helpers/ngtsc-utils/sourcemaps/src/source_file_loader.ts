...
export class SourceFileLoader {
  ...
  private loadSourceMap(
    ...
    if (external) {
      try {
        ...
        const externalMapPath = this.fs.resolve(
          this.fs.dirname(sourcePath),
          fileName
        );
        return {
  ...
  private processSources(
    ...
    return map.sources.map((source, index) => {
      const path = this.fs.resolve(
        sourceRoot,
        this.replaceSchemeWithPath(source)
      );
  ...
  private trackPath(path: AbsoluteFsPath): void {
    if (this.currentPaths.includes(path)) {
      throw new Error(
        `Circular source file mapping dependency: ${this.currentPaths.join(
          ' -> '
        )} -> ${path}`
      );
    }
