...
export class SourceFile {
  ...
  getOriginalLocation(
    ...
    const locationSegment: SegmentMarker = {
      line,
      column,
      position,
      next: undefined,
  ...
  private flattenMappings(): Mapping[] {
    ...
    for (let mappingIndex = 0; mappingIndex < mappings.length; mappingIndex++) {
      ...
      for (
        ...
        const bToCmapping: Mapping =
          bSource.flattenedMappings[bToCmappingIndex];
...
export function parseMappings(
  ...
  for (
    let generatedLine = 0;
    generatedLine < rawMappings.length;
    generatedLine++
  ) {
    ...
    for (const rawMapping of generatedLineMappings) {
      if (rawMapping.length >= 4) {
        ...
        const name =
          rawMapping.length === 5 ? rawMap.names[rawMapping[4]] : undefined;
        ...
        const generatedSegment: SegmentMarker = {
          ...
          position:
            generatedSourceStartOfLinePositions[generatedLine] +
            generatedColumn,
          next: undefined,
        };
 ...
 */
export function extractOriginalSegments(
  mappings: Mapping[]
): Map<SourceFile, SegmentMarker[]> {
  ...
  originalSegments.forEach((segmentMarkers) =>
    segmentMarkers.sort(compareSegments)
  );
...
export function computeStartOfLinePositions(str: string) {
  ...
  for (let i = 0; i < lineLengths.length - 1; i++) {
    startPositions.push(
      startPositions[i] + lineLengths[i] + NEWLINE_MARKER_OFFSET
    );
  }
