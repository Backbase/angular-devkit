...
import { SourceFile, SourceFileLoader } from '../ngtsc-utils/sourcemaps';
...
export class MessageExtractor {
  ...
  constructor(
    ...
    this.loader = new SourceFileLoader(this.fs, this.logger, {
      webpack: basePath,
    });
  }
  ...
  extractMessages(filename: string): ɵParsedMessage[] {
    ...
    const sourceCode = this.fs.readFile(
  ...
  private updateSourceLocations(
    ...
    for (const message of messages) {
      if (message.location !== undefined) {
        message.location = this.getOriginalLocation(
          sourceFile,
          message.location
        );
      }
  ...
  private getOriginalLocation(
    ...
    const originalEnd = sourceFile.getOriginalLocation(
      location.end.line,
      location.end.column
    );
