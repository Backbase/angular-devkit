...
export function isLocalize(
  ...
  return (
    isNamedIdentifier(expression, localizeName) &&
    isGlobalIdentifier(expression)
  );
}
 ...
 */
export function isGlobalIdentifier(identifier: any) {
  return (
...
export function unwrapMessagePartsFromLocalizeCall(
  ...
  let cooked: any = call.get('arguments')[0];
...
export function unwrapLazyLoadHelperCall(
  ...
  if (returnedNode.isIdentifier()) {
    ...
    const initializer: any = declaration.path.get('init');
...
function getReturnedExpression(
  ...
  const bodyStatements: any[] = fn.get('body').get('body');
...
export function getLocation(
  ...
  const endLocation =
    (endPath && getFileFromPath(endPath) === file && endPath.node.loc) ||
    startLocation;
